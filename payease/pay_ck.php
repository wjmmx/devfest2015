...
*/
	...
	$v_mid="82370001";    //商户编号，和首信签约后获得,测试的商户编号444
	$v_date=date('His');
	...
	$v_rcvaddr=$v_mid; //收货人地址，使用商户编号代替
	$v_rcvtel=$_POST['v_rcvtel'];   //收货人电话
	$v_rcvpost=$_POST['v_rcvpost'];  //收货人邮箱
	$v_amount=$_POST['v_amount']; //订单金额
	$v_orderstatus="1";//配货状态:0-未配齐，1-已配
	$v_ordername='RSGChina2016';  //订货人姓名
	$v_moneytype="0";  //0为人民币，1为美元，2为欧元，3为英镑，4为日元，5为韩元，6为澳大利亚元，7为卢布(内卡商户币种只能为人民币)
	$v_url="http://sg.dev/payease/receive1.php"; //支付完成后的实时返回地址。支付完成后实时先向这个地址做返回?在此地囿下做接收银行返回的支付确认消息?详细的返回参数格式西(接口文档的第二部分或者代码示例的received1.php)
	//echo $v_oid."<br>";
	//echo $v_amount."<br>";
	//echo $v_rcvname."<br>";
	//echo $v_rcvaddr."<br>";
